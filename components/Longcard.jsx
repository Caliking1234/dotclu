      ...
      <div className="w-[90%] max-w-5xl mx-auto rounded-2xl border border-cyan-500/20 bg-slate-900 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-slate-800">
          <div className="flex-1 flex flex-col items-center justify-center py-10 px-8 gap-2">
            <p className="stat-number text-5xl md:text-6xl">{a}</p>
            <span className="text-xs text-slate-400 text-center mt-1 tracking-wide">{b}</span>
          </div>
          <div className="flex-1 flex flex-col items-center justify-center py-10 px-8 gap-2">
            <p className="stat-number text-5xl md:text-6xl">{c}</p>
            <span className="text-xs text-slate-400 text-center mt-1 tracking-wide">{d}</span>
          </div>
          <div className="flex-1 flex flex-col items-center justify-center py-10 px-8 gap-2">
            <p className="stat-number text-5xl md:text-6xl">{e}</p>
            <span className="text-xs text-slate-400 text-center mt-1 tracking-wide">{f}</span>
          </div>
        </div>
      </div>
    </div>
